...
### CDN 
```html
<script defer src="https://cdn.jsdelivr.net/npm/alias-css@latest/dist/aliascss.js"></script>
```
...
```shell
 npx alias-css --config
  ...
  ```shell
    ...
    css file for each file having valid aliascss class names at the same location of file with the same filename added with .css extension.
